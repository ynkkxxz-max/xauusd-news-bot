...
_SYSTEM_RULES = (
    ...
    "why_it_matters ត្រូវចាប់ផ្តើមដោយ 🔴 (ប្រសិនជាដំណឹងអាក្រក់/ហានិភ័យ/តានតឹង) ឬ 🟢 (ប្រសិនជាដំណឹងល្អ/វិជ្ជមាន/ធូរស្រាល) ឬ 🟡 (ដំណឹងមធ្យម) "
    "រួចពន្យល់ពីហេតុផលសេដ្ឋកិច្ចឱ្យបានច្បាស់លាស់។ "
    "xau_pressure ត្រូវចាប់ផ្តើមដោយ 👉 🟢 (Bullish/មាសឡើង) ឬ 👉 🔴 (Bearish/មាសចុះ) ឬ 👉 🟡 (Sideway) រួមទាំងការពន្យល់យ៉ាងច្បាស់។"
)
...
def breaking_prompt(title: str, description: str) -> str:
    return (
        ...
        f"ហាមឆ្លើយតបជាគំរូដដែលៗ ឬ generic! ត្រូវឆ្លុះបញ្ចាំងពីទិន្នន័យនៃព័ត៌មានជាក់ស្ដែង ១០០%។"
    )
